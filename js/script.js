...
function filterTasks() {
    ...
    const filteredText = document.getElementById('txt-filter').value;
    const taskFilter = tasks.filter(task => task.task.includes(filteredText));
    displayTasksWith(taskFilter)
}
// Function to show completed tasks
function showCompleted() {
    // Get the filter value
    const taskFilter = tasks.filter(task => task.completed);
    displayTasksWith(taskFilter)
}
// Function to show all tasks
function showAll() {
    displayTasksWith(tasks)
}
// Display given tasks
function displayTasksWith(taskFilter){
    const taskList = document.getElementById('task-list');
    taskList.innerHTML = ''; // Clear the current list
    taskFilter.forEach(element => {
        const taskItem = `
        <div class="flex justify-between items-center p-[8px] border-b">
            <div class="flex flex-col">
                <span class="text-lg">${element.task}</span>
